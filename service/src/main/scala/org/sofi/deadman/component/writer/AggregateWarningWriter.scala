...
final class AggregateWarningWriter(val id: String, val eventLog: ActorRef) extends TaskWriter with ActorLogging {
  ...
  private var batch: Vector[AggregateWarning] = Vector.empty
  ...
  def onEvent = {
    case TaskWarning(t, ttw) ⇒
      ...
      val warning = AggregateWarning(t.aggregate, t.key, ttw, t.entity, t.ttl, t.ts, System.currentTimeMillis(), tags)
      batch = batch :+ warning
  }
  ...
  override def onCommand: Receive = {
    case q: GetWarnings ⇒
      ...
      val _ = AggregateWarning.select(aggregate).map { result ⇒
